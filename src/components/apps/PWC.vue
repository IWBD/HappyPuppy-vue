...
export default {
  ...
  data () {
    return {
      member_email:this.isLogin ? this.isLogin : null,
      certify_number:null,
      ...
      email_disabled: this.isLogin ? true : false,
      errorMessages: '',
      formHasErrors: false, 
      loading: false,
    }
  ...
  methods:{
    ...
    dataReset(){  
      ...
      this.email_disabled = isLogin ? true : false;
    },
    //인증번호 요청
    sendCertifyNumber(){
      ...
      if(!email_field.hasError){ 
        ...
        this.$http.post ('/api/member/certify_number', item).then(res => {
          ...
          if(!res.data.code){
            ...
            email_field.errorBucket.push("가입되지 않은 사용자입니다.");
          }else{
            alert('인증번호가 전송되었습니다.');
            return;
          }
